...
daily_raw_24 <- httr::GET(daily_url_2024,add_headers( "token" = "faXCJntxwbKXOeuoFQazeGxtGykRnsQm"
) )
...
daily_raw_25 <- httr::GET(daily_url_2025,add_headers( "token" = "faXCJntxwbKXOeuoFQazeGxtGykRnsQm"
) )
...
snow_summary <- snow %>% group_by(winter_season) %>%  summarize(max_cumulative = max(snow_rolling), min_cumulative = min(snow_rolling)) %>% ungroup()


snow_top_10 <- snow_summary %>% top_n(10, max_cumulative) %>% arrange(-max_cumulative)

snow_smallest_10 <- snow_summary %>% slice_min( min_cumulative, n=10) %>% arrange(min_cumulative)
...
## 2023-2024 Anchorage snow depth compared to winters since 1952.
